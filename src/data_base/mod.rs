...
impl EntityDescription {
    ...
    fn from_fields(fields: BTreeMap<String, Arc<Box<TypeDescription>>>) -> EntityDescription {
        ...
        let reverse_ids_map =
            ids_map.iter().map(|(k, v)| (v.clone(), k.clone())).collect::<BTreeMap<String, u16>>();
        EntityDescription {
            count: count,
            fields: fields,
            ids_map: ids_map,
...
impl Table {
    fn json_to_entity(json: &rustless::json::JsonValue,
                      description: &EntityDescription)
                      -> Result<Entity, IoEntityError> {
        if json.is_object() {
            ...
            let res = json_object.map(|object| {
                ...
                let selected_values_keys =
                    selected_values.keys().map(|key| key.clone()).collect::<HashSet<String>>();
                ...
                let types_keys =
                    description.fields.keys().map(|key| key.clone()).collect::<HashSet<String>>();
                let unselected_typed_keys = &selected_values_keys ^ &types_keys;

                if !unselected_json_keys.is_empty() || !unselected_typed_keys.is_empty() {
                    Err(IoEntityError::Read("Found unselected json values = [".to_string() +
                    ...
                    let fields = selected_values.iter()
    ...
    fn entity_to_json(entity: &Entity,
                      entity_description: &EntityDescription)
                      -> Result<rustless::json::JsonValue, IoEntityError> {
        let json_object: BTreeMap<String, rustless::json::JsonValue> = entity.fields
            .iter()
        ...
        if json_object.len() != entity.fields.len() {
            ...
            Err(IoEntityError::Write("Not found field descriptions for some fields ".to_string() +
                                     unset.as_str()))
        } else {
    ...
    pub fn put(&self,
        ...
        let key_entity = try!(Table::json_to_entity(key, &self.description.key)
            .map_err(|err| PersistenceError::IoEntity(err)));
        let value_entity = try!(Table::json_to_entity(value, &self.description.value)
            .map_err(|err| PersistenceError::IoEntity(err)));
...
fn get_undefined_fields(entity_fields: &BTreeMap<String, Option<Arc<Box<TypeDescription>>>>)
                        -> Vec<String> {
    entity_fields.iter()
        .filter_map(|(k, v)| {
            match *v {
...
fn create_entity_description(view: &EntityDescriptionView,
                             type_descs: &BTreeMap<String, Arc<Box<TypeDescription>>>)
                             -> Result<EntityDescription, String> {
    let mut entity_fields = view.fields
        .iter()
        .map(|(k, v)| (k.clone(), type_descs.get(v).map(|type_desc| type_desc.clone())))
        .collect();
    let undefined_fields: Vec<String> = get_undefined_fields(&entity_fields);
    if undefined_fields.iter().next().is_some() {
        Err(undefined_fields.iter().fold(String::new(),
                                         |base, field_name| base + ", " + field_name.as_str()))
    } else {
        let entity_fields = entity_fields.iter_mut()
            .filter_map(move |(k, v)| v.clone().map(|value| (k.clone(), value)))
            .collect();
        Ok(EntityDescription::from_fields(entity_fields))
    }
}

fn create_table_description(view: &TableDescriptionView,
                            type_descs: &BTreeMap<String, Arc<Box<TypeDescription>>>)
                            -> TableDescription {
    TableDescription {
...
impl DataBaseManager {
            ...
            reader: Box::new(move |json| {
                match json.clone() {
                    rustless::json::JsonValue::String(value) => {
                        encode(&value.clone(), bincode::SizeLimit::Infinite).unwrap()
                    }
    ...
    pub fn add_type(&mut self, type_desc: TypeDescription) -> Result<(), String> {
        if !self.type_descriptions.contains_key(&type_desc.name) {
            ...
            Err("Type with name ".to_string() + type_desc.name.clone().as_str() +
                " already defined.")
        }
    }
    ...
    pub fn get_data(&self,
                    ...
                    key: &rustless::json::JsonValue)
                    -> Result<Option<rustless::json::JsonValue>, PersistenceError> {
        let table = try!(self.tables
            .find(table_name)
            .ok_or(PersistenceError::TableNotFound(table_name.clone())));
        table.get().get(key)
    }
}
